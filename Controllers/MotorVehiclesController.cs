...
namespace AssetManagementSystem.Controllers
{
    ...
    public class MotorVehiclesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(int id, MotorVehicle asset)
        {
            ...
            {
                if (!_context.MotorVehicles.Any(e => e.Id == id))
                    return NotFound($"Motor vehicle with ID {id} does not exist.");
                else
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                if (asset == null)
                    return NotFound($"Motor vehicle with ID {id} not found.");
        ...
        public async Task<IActionResult> GetAll()
        {
            try
            {
                var vehicles = await _context.MotorVehicles.ToListAsync();
                return Ok(vehicles);
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Error retrieving motor vehicles: {ex.InnerException?.Message ?? ex.Message}");
            }
        }
